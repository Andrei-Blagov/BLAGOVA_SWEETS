...
- После переключения каталога Nginx отдаёт Nuxt `200.html` для новых `/products/<slug>/`, чтобы опубликованный в админке товар открывался без новой сборки. Проверка deployment запрашивает `/menu/`, существующую и новую карточку, `/cart/` и `/checkout/`; это проверка маршрутов, не полноценный браузерный заказ.
