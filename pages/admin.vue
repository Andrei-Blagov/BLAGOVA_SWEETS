...
async function conversationAction(action: string) {
  ...
  try {
    ...
    notice.value = action === 'reply' ? 'Ответ записан в Supabase. Чат витрины пока работает отдельно в демо-режиме.' : 'Режим диалога сохранён.';
...
<template>
  ...
  <div v-else class="studio">
    <aside class="studio-sidebar">
      ...
      <nav aria-label="Разделы админки"><button v-for="item in tabs" :key="item.id" :class="{active:tab===item.id}" :aria-current="tab===item.id ? 'page' : undefined" @click="tab=item.id; notice=''">
    ...
    <main class="studio-main">
      ...
      <div class="studio-content">
        ...
        <div class="studio-banner"><AtelierIcon name="leaf"/><p><strong>Подключено к Supabase.</strong> Здесь данные общей базы. Заказы и чат на витрине пока остаются локальным демо. LINE и Google Calendar не подключены.</p></div>
        ...
        <template v-else>
          <template v-if="tab==='orders'">
            ...
            <div class="studio-orders-layout"><section class="studio-card">
              ...
              <div class="studio-order-list"><button v-for="o in filtered" :key="o.id" :class="['studio-order-row',{selected:selectedId===o.id}]" @click="selectedId=o.id"><div><small>{{ o.id.slice(0,8).toUpperCase() }} · {{ o.is_demo ? 'Тестовый' : 'Заказ' }}</small><strong>{{ o.customer_name }}</strong><span>{{ o.order_items.map(i=>`${i.product_name} × ${i.quantity}`).join(', ') }}</span></div><div class="studio-order-meta"><span :class="['status-chip','status-'+o.status]">{{ label(o.status) }}</span><strong>{{ money(orderTotal(o)) }}</strong><small>{{ dateTime(o.scheduled_start) }}</small></div></button>
                <div v-if="!filtered.length" class="studio-empty"><h3>{{ orderCount ? 'Ничего не найдено' : 'Первый заказ ещё впереди.' }}</h3><p>{{ orderCount ? 'Попробуйте другой фильтр.' : 'База подключена. Локальные демо-заявки сюда не переносятся.' }}</p></div>
              </div>
          ...
          <section v-if="tab==='integrations'" class="integration-grid"><article class="studio-card integration-card"><span class="status-chip status-confirmed">Подключено</span><h2>Supabase</h2><p>Вход сотрудников, роли, заказы, переписка и материалы. База во Франкфурте.</p></article><article v-for="name in ['Google Calendar','LINE','AI / RAG']" :key="name" class="studio-card integration-card"><span class="integration-off">Не подключено</span><h2>{{ name }}</h2><p>Следующий этап подключения. Автоматические отправки сейчас отключены.</p></article></section>
        </template>
