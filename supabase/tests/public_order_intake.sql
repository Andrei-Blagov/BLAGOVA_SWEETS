...
do $$ begin
  begin
    perform public.receive_storefront_order(
      (select request_key from intake_test_ids),'website','ru','QA','qa@example.invalid','pickup','',0,
      now()+interval '5 days',now()+interval '5 days 2 hours','',
      '[{"name":"QA cake","detail":"1 kg","quantity":1,"unit_price_minor":145000}]'::jsonb,
...
select * from public.receive_storefront_order(
  (select request_key from intake_test_ids),'chat','ru','QA','qa@example.invalid','pickup','',0,
  now()+interval '5 days',now()+interval '5 days 2 hours','QA',
  '[{"name":"QA cake","detail":"1 kg","quantity":1,"unit_price_minor":145000}]'::jsonb,
