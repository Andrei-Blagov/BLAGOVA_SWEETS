...
test('storefront uses only active published variants and the primary image', () => {
  const rows = [
    ...
    { slug:'berry',category:'cake',status:'published',image_path:'/old.webp',name:{ru:'Ягода',en:'Berry',th:'เบอร์รี'},
      ...
      product_images:[{is_primary:true,storage_path:'products/p/photo.webp',alt:{ru:'Ягодный торт',en:'Berry cake',th:'เค้กเบอร์รี'}}] }
  ];
  ...
  assert.equal(products[0].imageAlt.en,'Berry cake');
  assert.equal(products[0].imageAlt.th,'เค้กเบอร์รี');
  ...
  assert.equal(mapPublishedCatalog([{...rows[1],product_images:[]}],path => path)[0].imageAlt.en,'Berry');
